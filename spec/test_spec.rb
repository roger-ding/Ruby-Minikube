...
describe StringOps do
  ...
  describe "#count" do
    it "returns character count for input_string" do
      expected_count = 130
      expect(string_ops.count(input_string)).to eq(expected_count)
    end
  end
end
...
context "mock response" do
  it "mocks the response from to_lower" do
    mocked_response = "This is a mocked to_lower response."
    mocked_string_ops = double("mocked_string_ops")
    allow(mocked_string_ops).to receive(:to_lower).and_return(mocked_response)
    expect(mocked_string_ops.to_lower("any string")).to eq(mocked_response)
  end
end
